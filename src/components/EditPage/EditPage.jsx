        ...
        <div id='container'>
            <div id='form'>
                <form onSubmit={(event) => handleSubmit(event, editTask)}>
                    <div id="newTaskInput">
                        <h2>Task</h2>
                        <TextField id="outlined-basic-task" label="" variant="outlined" type='text' value={editTask.task} onChange={(event) => handleChange(event, 'task')} />
                    </div>
            
                    <div id="dateInput">
                        <h2>Due Date</h2>
                        <TextField id=""label="" variant="outlined" placeholder='Due Date' type='date' value={editTask.due_date?.split(`T`)[0]} onChange={(event) => handleChange(event, 'due_date')} />
                    </div>

                    <div id="importanceInput">
                        <h2>Importance</h2>
                        <Slider id="slider" defaultValue={50} min={1} max={99} type='int' value={editTask.importance} onChange={(event) => handleChange(event, 'importance')}/>
                    </div>

                    <div id="timeInput">
                        <h2>Time Requirement</h2>
                        <TextField id="" label="" type='int' value={editTask.time_requirement} onChange={(event) => handleChange(event, 'time_requirement')} />
                    </div>
           
                    <div id ="notesInput">
                        <h2>Notes</h2>
                        <TextField fullWidth label='' multiline rows={4} value={editTask.notes} onChange={(event) => handleChange(event, 'notes')}/>
                    </div>
